 ...
 * A bearish reversal pattern with both support and resistance lines rising but converging
 */

export interface RisingWedgePattern {
  type: 'rising_wedge';
  direction: 'bearish';
...
export interface RisingWedgeConfig {
  minTouches: number;
  minPatternBars: number;
  minRSquared: number;
  minConvergenceRate: number;
  maxSlopeRatio: number; // Upper slope should be less than lower but not by too much
  adaptivePivotWindow: boolean;
}

const DEFAULT_CONFIG: RisingWedgeConfig = {
  minTouches: 3, // Increased from 2
  minPatternBars: 15, // Increased from 10
  minRSquared: 0.80,
  minConvergenceRate: 0.0005,
  maxSlopeRatio: 0.9, // Upper slope should be < 90% of lower
...
export function detectRisingWedge(
  ...
  lows: number[],
  closes: number[],
  config: Partial<RisingWedgeConfig> = {}
): RisingWedgePattern[] {
  const cfg = { ...DEFAULT_CONFIG, ...config };
  const patterns: RisingWedgePattern[] = [];
  ...
  for (let startIdx = 0; startIdx < Math.min(pivotHighs.length - 1, pivotLows.length - 1); startIdx++) {
    ...
    // Find all pivots in a reasonable range for trendline calculation
    for (let endOffset = 1; endOffset < Math.min(5, pivotHighs.length - startIdx, pivotLows.length); endOffset++) {
      const rangeEnd = upperStart.index + 50; // Look up to 50 bars ahead
      ...
      // Lower line must rise faster (converging) - upper slope should be less than lower
      const slopeRatio = upperTrendline.slope / lowerTrendline.slope;
      ...
      const patternHeight = (upperTrendline.slope * upperStart.index + upperTrendline.intercept) -
                           (lowerTrendline.slope * lowerStart.index + lowerTrendline.intercept);
      const target = lowerStart.price - patternHeight;
      ...
      break; // Found valid pattern, move to next start
    }
  }
...
function removeDuplicatePatterns(patterns: RisingWedgePattern[]): RisingWedgePattern[] {
  if (patterns.length <= 1) return patterns;
  
  patterns.sort((a, b) => b.confidence - a.confidence);
  
  const result: RisingWedgePattern[] = [];
