...
} ): TrendStrength
{
  ...
  const currentPrice = close[ length - 1 ] ?? 0;
  const currentRsi = rsi[ length - 1 ] ?? 50;
  const currentStochK = stochK[ length - 1 ] ?? 50;
  const currentAdx = adx[ length - 1 ] ?? 0;

  const ema200Value = ema200[ length - 1 ] ?? 0;
  const ema50Value = ema50[ length - 1 ] ?? 0;
  const sma20Value = sma20[ length - 1 ] ?? 0;
  const ema9Value = ema9[ length - 1 ] ?? 0;
  const ema21Value = ema21[ length - 1 ] ?? 0;
  const macdHistValue = macdHistogram[ length - 1 ] ?? 0;

  let regime: TrendStrength[ "regime" ] = "range";
  ...
  const notes: string[] = [];
  const components: TrendStrength[ "components" ] = {};

  let bullishSum = 0;
  let bearishSum = 0;

  const registerComponent = (
    name: keyof typeof baseWeights,
    direction: "bullish" | "bearish" | "neutral",
    strength: number,
  ...
  ) =>
  {
    ...
    {
      bullishSum += weight * normalizedStrength;
      positives.push( bullishReason );
    } else if ( direction === "bearish" )
    {
      bearishSum += weight * normalizedStrength;
      negatives.push( bearishReason );
    }

    components[ name ] = {
      direction,
      strength: Number( normalizedStrength.toFixed( 3 ) ),
  ...
  if ( ema200Value > 0 )
  {
    const emaDiff = ( currentPrice - ema200Value ) / ema200Value;
    const emaStrength = clamp( Math.abs( emaDiff ) / 0.02 );
    registerComponent(
      "ema200",
      emaDiff >= 0 ? "bullish" : "bearish",
      emaStrength,
      "السعر فوق EMA200",
      "السعر تحت EMA200"
    );
  ...
  {
    registerComponent( "ema200", "neutral", 0, "", "" );
  }

  if ( ema50Value > 0 && sma20Value > 0 )
  {
    const maDiff = ( ema50Value - sma20Value ) / sma20Value;
    const maStrength = clamp( Math.abs( maDiff ) / 0.01 );
    registerComponent(
      "maTrend",
      ema50Value >= sma20Value ? "bullish" : "bearish",
      maStrength,
      "EMA50 فوق SMA20",
      "EMA50 تحت SMA20"
    );
  ...
  {
    registerComponent( "maTrend", "neutral", 0, "", "" );
  }

  if ( ema21Value > 0 )
  {
    const emaShortDiff = ( ema9Value - ema21Value ) / ema21Value;
    const emaShortStrength = clamp( Math.abs( emaShortDiff ) / 0.006 );
    registerComponent(
      "emaShort",
      ema9Value >= ema21Value ? "bullish" : "bearish",
      emaShortStrength,
      "EMA9 فوق EMA21",
      "EMA9 تحت EMA21"
    );
  ...
  {
    registerComponent( "emaShort", "neutral", 0, "", "" );
  }

  if ( macdHistValue > 0 )
  {
    const macdStrength = clamp( Math.abs( macdHistValue ) / Math.max( currentPrice * 0.002, 1e-9 ) );
    registerComponent( "macd", "bullish", macdStrength, "MACD هيستوجرام موجب", "" );
  } else if ( macdHistValue < 0 )
  {
    const macdStrength = clamp( Math.abs( macdHistValue ) / Math.max( currentPrice * 0.002, 1e-9 ) );
    registerComponent( "macd", "bearish", macdStrength, "", "MACD هيستوجرام سالب" );
  } else
  {
    registerComponent( "macd", "neutral", 0, "", "" );
  }

  const rsiStrength = clamp( Math.abs( currentRsi - 50 ) / 25 );
  if ( currentRsi >= 55 )
  {
    registerComponent( "rsi", "bullish", rsiStrength, "RSI يميل للصعود", "" );
    if ( currentRsi >= 70 ) notes.push( "RSI تشبع شرائي" );
  } else if ( currentRsi <= 45 )
  {
    registerComponent( "rsi", "bearish", rsiStrength, "", "RSI يميل للهبوط" );
    if ( currentRsi <= 30 ) notes.push( "RSI تشبع بيعي" );
  } else
  {
    registerComponent( "rsi", "neutral", 0, "", "" );
  }

  const stochStrength = clamp( Math.abs( currentStochK - 50 ) / 40 );
  if ( currentStochK >= 60 )
  {
    registerComponent( "stoch", "bullish", stochStrength, "StochRSI صاعد", "" );
    if ( currentStochK >= 80 ) notes.push( "StochRSI تشبع شرائي" );
  } else if ( currentStochK <= 40 )
  {
    registerComponent( "stoch", "bearish", stochStrength, "", "StochRSI هابط" );
    if ( currentStochK <= 20 ) notes.push( "StochRSI تشبع بيعي" );
  } else
  {
    registerComponent( "stoch", "neutral", 0, "", "" );
  }

  let obvTrend = 0;
  if ( obv.length > 1 )
  {
    const startIdx = Math.max( 0, obv.length - 11 );
    obvTrend = obv[ obv.length - 1 ] - obv[ startIdx ];
  }

  if ( obvTrend > 0 )
  {
    registerComponent( "obv", "bullish", 0.7, "OBV يدعم الصعود", "" );
  } else if ( obvTrend < 0 )
  {
    registerComponent( "obv", "bearish", 0.7, "", "OBV يدعم الهبوط" );
  } else
  {
    registerComponent( "obv", "neutral", 0, "", "" );
  }

  if ( regime === "range" )
  {
    notes.push( "الاتجاه ضعيف (ADX منخفض)" );
  }
  ...
  let score = totalWeight > 0 ? ( ( bullishSum - bearishSum ) / totalWeight ) * 100 : 0;

  const trendBias = score > 5 ? "bullish" : score < -5 ? "bearish" : "neutral";
  let trendStrengthValue = Math.abs( score ) * ( 0.5 + 0.5 * clamp( ( currentAdx - 10 ) / 30 ) );

  const momentumNet =
    components.macd?.strength! * ( components.macd?.direction === "bullish" ? 1 : -1 ) +
    components.rsi?.strength! * ( components.rsi?.direction === "bullish" ? 1 : -1 ) +
    components.stoch?.strength! * ( components.stoch?.direction === "bullish" ? 1 : -1 );

  const trendNet =
    components.ema200?.strength! * ( components.ema200?.direction === "bullish" ? 1 : -1 ) +
    components.maTrend?.strength! * ( components.maTrend?.direction === "bullish" ? 1 : -1 ) +
    components.emaShort?.strength! * ( components.emaShort?.direction === "bullish" ? 1 : -1 );

  if ( momentumNet * trendNet < -0.05 )
  {
    score *= 0.85;
    trendStrengthValue *= 0.85;
    notes.push( "تعارض بين الزخم والاتجاه العام" );
  }

  const confidence = clamp( ( currentAdx / 40 ) * ( 0.6 + 0.4 * ( Math.abs( score ) / 100 ) ) );

  return {
    bullishScore: Number( Math.min( 100, bullishScore ).toFixed( 2 ) ),
    bearishScore: Number( Math.min( 100, bearishScore ).toFixed( 2 ) ),
    trend: trendBias,
    strength: clamp( trendStrengthValue / 100 ),
    confidence,
...
export function analyzeTimeframe ( data: OHLCVData, symbol: string, timeframe: string ): FullAnalysis
{
  ...
  const trendStrength = buildTrendStrengthSnapshot( {
    close,
    rsi,
  ...
  const addSignal = (
    list: SignalStrength[ "buySignals" ] | SignalStrength[ "sellSignals" ],
    indicator: string,
    strength: number,
    description: string
  ) =>
  {
    if ( !list.some( s => s.indicator === indicator ) )
    {
      list.push( { indicator, strength, description } );
    }
  };

  trendStrength.positives?.forEach( reason =>
  {
    addSignal( buySignals, reason, 0.7, reason );
  } );
  trendStrength.negatives?.forEach( reason =>
  {
    addSignal( sellSignals, reason, 0.7, reason );
  } );

  if ( currentRsi < 30 )
  {
    addSignal( buySignals, "RSI", 0.8, "منطقة تشبع بيعي" );
  } else if ( currentRsi > 70 )
  {
    addSignal( sellSignals, "RSI", 0.8, "منطقة تشبع شرائي" );
  }

  if ( currentMacd > currentMacdSignal && macd[ length - 2 ] <= macdSignal[ length - 2 ] )
  {
    addSignal( buySignals, "MACD", 0.75, "تقاطع إيجابي" );
  } else if ( currentMacd < currentMacdSignal && macd[ length - 2 ] >= macdSignal[ length - 2 ] )
  {
    addSignal( sellSignals, "MACD", 0.75, "تقاطع سلبي" );
  }

  if ( currentStochK < 20 && stochK[ length - 2 ] < stochD[ length - 2 ] && currentStochK > stochD[ length - 1 ] )
  {
    addSignal( buySignals, "Stoch RSI", 0.7, "تقاطع من منطقة تشبع" );
  } else if ( currentStochK > 80 && stochK[ length - 2 ] > stochD[ length - 2 ] && currentStochK < stochD[ length - 1 ] )
  {
    addSignal( sellSignals, "Stoch RSI", 0.7, "تقاطع من منطقة تشبع" );
  }

  if ( currentPrice < bbLower[ length - 1 ] )
  {
    addSignal( buySignals, "Bollinger", 0.65, "السعر تحت الحد السفلي" );
  } else if ( currentPrice > bbUpper[ length - 1 ] )
  {
    addSignal( sellSignals, "Bollinger", 0.65, "السعر فوق الحد العلوي" );
  }
  ...
  if ( nearestSupport && ( currentPrice - nearestSupport.price ) / currentPrice < 0.01 )
  {
    addSignal(
      ...
      `اختبار دعم عند ${ nearestSupport.price.toFixed( 2 ) }`
    );
  }
  ...
  if ( nearestResistance && ( nearestResistance.price - currentPrice ) / currentPrice < 0.01 )
  {
    addSignal(
      ...
      `اختبار مقاومة عند ${ nearestResistance.price.toFixed( 2 ) }`
    );
  }
  ...
  if ( volume[ length - 1 ] > avgVolume * 1.5 )
  {
    ...
    {
      addSignal( buySignals, "Volume", 0.6, "حجم تداول عالي مع شمعة صاعدة" );
    } else
    {
      addSignal( sellSignals, "Volume", 0.6, "حجم تداول عالي مع شمعة هابطة" );
    }
  }
  ...
  const direction = trendStrength.trend === "bullish" ? "long" : trendStrength.trend === "bearish" ? "short" : "neutral";
  const entry = currentPrice;
  const riskBuffer = currentPrice * 0.015;
  const stopLoss = direction === "long" ?
    ( nearestSupport?.price || currentPrice - riskBuffer ) :
    ( nearestResistance?.price || currentPrice + riskBuffer );
