/**
 * 🟢 Crypto.com Spot Exchange Integration
...
export const websocketConfig = {
  ...
  subscribe: (channel: string, symbol: string) => {
    const cryptoSymbol = symbol.replace('/', '_');
    
    return JSON.stringify({
      id: Date.now(),
      method: 'subscribe',
      params: {
        channels: [`${channel}.${cryptoSymbol}`],
      },
    });
  },
  ...
  parseMessage: (data: any) => {
    if (!data || !data.result || !data.result.channel) return null;
    ...
    if (channel.startsWith('ticker')) {
      return {
        type: 'ticker',
        data: {
          symbol: channel.split('.')[1].replace('_', '/'),
          last: parseFloat(d.a),
          bid: parseFloat(d.b),
          ask: parseFloat(d.k),
          high: parseFloat(d.h),
