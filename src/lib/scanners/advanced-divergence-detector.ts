...
class IndicatorCalculator {
  ...
  static calculateStochRSI(candles: OHLCV[], rsiPeriod: number = 14, stochPeriod: number = 14): number[] {
    ...
    for (let i = 0; i < rsi.length; i++) {
      ...
      stochRSI.push(((rsi[i] - min) / range) * 100);
    }
...
export class AdvancedDivergenceDetector {
  ...
  private getAdaptivePivotLookback(timeframe: string): number {
    if (this.config.timeframeAdaptive) {
      ...
      if (tf.includes('1d') || tf.includes('3d') || tf.includes('d')) return 3;
  ...
  private getAdaptivePivotRange(timeframe: string): number {
    if (this.config.timeframeAdaptive) {
      ...
      if (tf.includes('1d') || tf.includes('3d') || tf.includes('d')) return 4;
  ...
  private analyzePair(
    ...
    if (direction === 'bullish') {
      ...
      if (priceChange < -0.003 && indicatorChange > 0.005) {  // ← تخفيف
        type = 'strong';
        baseScore = 90;
      }
      // 📊 Medium Bullish: Equal/Slight Lower Low (price) + Higher Low (indicator)
      else if (priceChange < 0.005 && priceChange > -0.03 && indicatorChange > 0.005) {  // ← تخفيف
        type = 'medium';
        baseScore = 70;
      }
      // 📉 Weak Bullish: Lower Low (price) + Equal/Slight Higher Low (indicator)
      else if (priceChange < -0.005 && indicatorChange > -0.01 && indicatorChange < 0.02) {  // ← تخفيف
        type = 'weak';
        baseScore = 50;
      }
      // 🔄 Hidden Bullish: Higher Low (price) + Lower Low (indicator) - Continuation
      else if (priceChange > 0.003 && indicatorChange < -0.005) {  // ← تخفيف
        type = 'hidden';
        baseScore = 75;
      }
      // ⚡ Exaggerated Bullish: Equal Lows (price) + Significantly Higher Low (indicator)
      else if (Math.abs(priceChange) < 0.005 && indicatorChange > 0.02) {  // ← تخفيف
        type = 'exaggerated';
        baseScore = 65;
      }
      // ↩️ Reverse Bullish: Lower Low (price) + Lower Low (indicator) but indicator shows accumulation
      else if (priceChange < -0.005 && indicatorChange < -0.003 && this.checkAccumulation(candles, point1.index, point2.index)) {  // ← تخفيف
        type = 'reverse';
        baseScore = 55;
      }
      ...
      else if (priceChange > 0.005 && indicatorChange > 0.003 && this.checkDistribution(candles, point1.index, point2.index)) {  // ← تخفيف
        type = 'reverse';
        baseScore = 55;
      }
    }
    ...
    if (this.config.strictMode && finalScore < 40) {  // ← تخفيف: كان 60
      return null;
    }
