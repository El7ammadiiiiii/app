...
const getHeightClass = ( percent: number ) =>
{
  const clamped = Math.min( 100, Math.max( 0, percent ) );
  const index = Math.min( 20, Math.max( 0, Math.round( clamped / 5 ) ) );
    ...
    <div className="min-h-screen bg-gray-900 text-white p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-gray-800 rounded-xl p-6">
          ...
          <div className="space-y-4">
            <div>
              <label htmlFor="model-type-select" className="text-sm text-gray-400">Model Architecture</label>
              <select
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="epochs-input" className="text-sm text-gray-400">Epochs</label>
                <input
                  id="epochs-input"
                  type="number"
                  className="w-full bg-gray-700 rounded-lg p-2 mt-1"
              ...
              <div>
                <label htmlFor="batch-size-input" className="text-sm text-gray-400">Batch Size</label>
                <input
                  id="batch-size-input"
                  type="number"
                  className="w-full bg-gray-700 rounded-lg p-2 mt-1"
            ...
            <div>
              <label htmlFor="learning-rate-input" className="text-sm text-gray-400">Learning Rate</label>
              <input
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="h-3 bg-gray-700 rounded-full overflow-hidden">
                <div
                  className={ `h-full bg-gradient-to-r from-purple-500 to-pink-500 transition-all duration-300 ${ getWidthClass( progressPercent ) }` }
                />
              </div>
            </div>
            ...
            <div className="flex gap-3">
              <button
                ...
                className={ `flex-1 flex items-center justify-center gap-2 py-3 rounded-lg font-medium transition ${ progress.status === 'training'
        ...
        <div className="bg-gray-800 rounded-xl p-6">
          ...
          <div className="h-48 flex items-end gap-1">
            { lossHistory.slice( -50 ).map( ( loss, i ) => (
              <div
                key={ i }
                className={ `flex-1 bg-red-500 rounded-t opacity-80 ${ getHeightClass( Math.min( ( loss / 3 ) * 100, 100 ) ) }` }
              />
            ) ) }
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-xl p-6">
          ...
          <div className="h-48 flex items-end gap-1">
            { accHistory.slice( -50 ).map( ( acc, i ) => (
              <div
                key={ i }
                className={ `flex-1 bg-green-500 rounded-t opacity-80 ${ getHeightClass( acc ) }` }
              />
            ) ) }
          </div>
        </div>
