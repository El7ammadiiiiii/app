...
export function renderSymmetricalTriangle(
  ...
  const color = getPatternColor(pattern.name || 'Symmetrical Triangle');
  ...
  series.push(
    ...
    {
      type: 'scatter',
      data: [ { value: [ extendedEnd, (hXY + lXY) / 2 ], label: { show: true, formatter: pattern.name || 'Symmetrical Triangle', position: 'right', fontSize: 10, color: '#fff', backgroundColor: color + '99', padding: [ 2, 6 ], borderRadius: 3 }, itemStyle: { color: 'transparent', borderWidth: 0 } } ],
