...
export function renderBullishPennant(
  ...
  const color = getPatternColor(pattern.name || 'Bullish Pennant');
  ...
  if (pole) {
    series.push({
      type: 'line', data: buildStraightLineData(total, pole.start.index, pole.start.price, pole.end.index, pole.end.price),
      connectNulls: true, xAxisIndex: 0, yAxisIndex: 0, symbol: 'none',
  ...
  series.push(
    ...
    {
      type: 'scatter',
      data: [ { value: [ extendedEnd, (hXY + lXY) / 2 ], label: { show: true, formatter: pattern.name || 'Bullish Pennant', position: 'right', fontSize: 10, color: '#fff', backgroundColor: color + '99', padding: [ 2, 6 ], borderRadius: 3 }, itemStyle: { color: 'transparent', borderWidth: 0 } } ],
