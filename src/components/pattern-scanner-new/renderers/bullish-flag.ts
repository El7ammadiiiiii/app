...
export function renderBullishFlag(
  ...
  const color = getPatternColor(pattern.name || 'Bullish Flag');
  ...
  series.push(
    ...
    },
    {
      ...
      data: pattern.pivots.map((p) => ({
        ...
        itemStyle: { color: p.dir === 1 ? '#00E676' : '#FF5252', borderColor: '#fff', borderWidth: 1.5 },
      })),
      xAxisIndex: 0,
      yAxisIndex: 0,
      symbolSize: 8,
    ...
    },
    {
      ...
      data: [ {
        ...
        label: {
          show: true,
          formatter: pattern.name || 'Bullish Flag',
          position: 'right',
          fontSize: 10,
          color: '#fff',
  ...
  if (pattern.breakout?.detected && pattern.breakout.index >= 0) {
    series.push({
      type: 'scatter',
      data: [ { value: [ pattern.breakout.index, pattern.breakout.price ], itemStyle: { color: 'rgba(66,165,245,0.25)', borderColor: '#42A5F5', borderWidth: 2.5 } } ],
      xAxisIndex: 0,
      yAxisIndex: 0,
      symbolSize: 20,
      symbol: 'circle',
