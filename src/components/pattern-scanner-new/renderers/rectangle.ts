...
export function renderRectangle(
  ...
  const color = getPatternColor(pattern.name || 'Ranging Channel');
  ...
  const topY = regY(highReg.slope, highReg.intercept, midX);
  const lowY = regY(lowReg.slope, lowReg.intercept, midX);

  series.push(
    {
      type: 'line',
      data: buildStraightLineData(total, startIdx, topY, extendedEnd, topY),
      connectNulls: true,
      xAxisIndex: 0,
      yAxisIndex: 0,
      symbol: 'none',
    ...
    {
      type: 'line',
      data: buildStraightLineData(total, startIdx, lowY, extendedEnd, lowY),
      connectNulls: true,
      xAxisIndex: 0,
      yAxisIndex: 0,
      symbol: 'none',
  ...
  series.push(
    ...
    },
    {
      type: 'scatter',
      data: [ {
        value: [ extendedEnd, (topY + lowY) / 2 ],
        label: {
          show: true,
          formatter: pattern.name || 'Ranging Channel',
          position: 'right',
          fontSize: 10,
          color: '#fff',
