...
export function renderDescendingChannel(
  ...
  const color = getPatternColor(pattern.name || 'Descending Channel');
  ...
  const midX = (startIdx + safeEnd) / 2;
  const avgSlope = (highReg.slope + lowReg.slope) / 2;
  const highMid = regY(highReg.slope, highReg.intercept, midX);
  const lowMid = regY(lowReg.slope, lowReg.intercept, midX);
  const highIntercept = highMid - avgSlope * midX;
  const lowIntercept = lowMid - avgSlope * midX;
  ...
  series.push(
    {
      type: 'line',
      data: buildStraightLineData(total, startIdx, hSY, extendedEnd, hXY),
      connectNulls: true,
      xAxisIndex: 0,
      yAxisIndex: 0,
      symbol: 'none',
    ...
    {
      type: 'line',
      data: buildStraightLineData(total, startIdx, lSY, extendedEnd, lXY),
      connectNulls: true,
      xAxisIndex: 0,
      yAxisIndex: 0,
      symbol: 'none',
  ...
  for (const p of sortedPivots) {
    if (p.index >= 0 && p.index < total) zzData[p.index] = p.price;
  }
  ...
  series.push(
    {
      ...
      yAxisIndex: 0,
      symbol: 'none',
      lineStyle: { color: zigzagColor, width: 1, type: 'solid' as const },
      silent: true,
      z: 9,
      animation: false,
    },
    {
      type: 'scatter',
      data: pattern.pivots.map((p) => ({
        value: [ p.index, p.price ],
        itemStyle: {
          color: p.dir === 1 ? '#00E676' : '#FF5252',
          borderColor: '#fff',
          borderWidth: 1.5,
        },
      })),
    ...
    },
    {
      type: 'scatter',
      data: [ {
        value: [ extendedEnd, (hXY + lXY) / 2 ],
        label: {
          show: true,
          formatter: pattern.name || 'Descending Channel',
          position: 'right',
          fontSize: 10,
          color: '#fff',
  ...
  if (pattern.breakout?.detected && pattern.breakout.index >= 0) {
    series.push({
      type: 'scatter',
      data: [ {
        value: [ pattern.breakout.index, pattern.breakout.price ],
        itemStyle: { color: 'rgba(66,165,245,0.25)', borderColor: '#42A5F5', borderWidth: 2.5 },
      } ],
      xAxisIndex: 0,
      yAxisIndex: 0,
      symbolSize: 20,
