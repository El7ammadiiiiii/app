...
export function SubIndicatorChart({ type, data, height = DEFAULT_HEIGHT, onClose }: SubChartProps) {
  ...
  return (
    <div 
      ref={containerRef}
      className="relative border-t border-[#363a45]"
      style={{ background: 'linear-gradient(90deg, #030508, #0d3b3b)' }}
    >
      {/* Y-Axis interaction zone (Left side) - تكبير/تصغير عمودي */}
      <div
        className={`absolute inset-y-3 left-0 w-12 cursor-ns-resize z-40 flex items-center justify-center 
      ...
      <div
        className={`absolute left-12 right-5 h-4 cursor-ew-resize z-40 
      ...
      <div
        className={`absolute top-0 left-0 right-0 h-3 cursor-ns-resize z-30 
      ...
      <div
        className={`absolute bottom-0 left-0 right-0 h-3 cursor-ns-resize z-30 
          ${isDragging ? 'bg-blue-500/50' : 'hover:bg-blue-500/40'}`}
        onMouseDown={startDrag}
        style={{ touchAction: 'none' }}
      >
