...
function detectOrderBlocks(data: CandleData[], settings: Required<SMCSettings>): OrderBlock[] {
  ...
  for (let i = 2; i < data.length - 1; i++) {
    ...
    if (prev.close < prev.open) {  // Previous was bearish
      ...
      if (bearishSize > avgSize * 1.5) {  // Strong bearish
...
function orderBlocksToGraphic(blocks: OrderBlock[], dataLength: number): any[] {
...
function fvgToMarkArea(gaps: FairValueGap[]): any[] {
...
function marketStructureToMarkPoint(points: MarketStructurePoint[]): any[] {
  return points.map(p => ({
    name: p.type,
...
function liquidityToMarkLine(zones: LiquidityZone[], dataLength: number): any[] {
