...
}: MultiLineChartProps )
{
    ...
    const render = useCallback( () =>
    {
        ...
        const allValues = parsed.flatMap( ( d ) => series.map( ( s ) => d[ s.key ] || 0 ) );
        const x = d3.scaleTime().domain( d3.extent( parsed, ( d ) => d.date ) as [ Date, Date ] ).range( [ 0, w ] );
        const y = d3.scaleLinear().domain( [ 0, d3.max( allValues ) || 0 ] ).nice().range( [ h, 0 ] );
        ...
        series.forEach( ( s ) =>
        {
            const lineGen = d3.line<any>().x( ( d ) => x( d.date ) ).y( ( d ) => y( d[ s.key ] || 0 ) ).curve( d3.curveMonotoneX );

            const linePath = g.append( "path" ).datum( parsed ).attr( "fill", "none" )
                .attr( "stroke", s.color ).attr( "stroke-width", strokeWidth ).attr( "d", lineGen );
        ...
        if ( showLegend )
        {
            ...
            series.forEach( ( s ) =>
            {
                ...
                const text = item.append( "text" ).attr( "x", 18 ).attr( "y", 4 )
        ...
        series.forEach( ( s ) =>
        {
            crosshair.append( "circle" ).attr( "class", `c-${ s.key }` ).attr( "r", 4 )
        ...
        const showTooltip = ( event: any ) =>
        {
            ...
            series.forEach( ( s ) =>
            {
                crosshair.select( `.c-${ s.key }` ).attr( "cx", x( d.date ) ).attr( "cy", y( d[ s.key ] || 0 ) );
            } );

            const rows = series.map( ( s ) => `<div style="display:flex;align-items:center;gap:6px"><span style="width:8px;height:8px;border-radius:50%;background:${ s.color };display:inline-block"></span>${ s.label }: <span style="font-weight:600">${ fmtY( d[ s.key ] || 0 ) }</span></div>` ).join( "" );
    ...
    }, [ data, series, height, yAxisLabel, showGrid, showLegend, strokeWidth, formatY ] );
