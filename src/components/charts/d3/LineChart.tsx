...
}: LineChartProps )
{
    ...
    const render = useCallback( () =>
    {
        ...
        const curveType = curve === "monotone" ? d3.curveMonotoneX : curve === "step" ? d3.curveStep : d3.curveLinear;
        ...
        // Line
        const lineGen = d3.line<any>().x( ( d ) => x( d.date ) ).y( ( d ) => y( d.value ) ).curve( curveType );

        const linePath = g.append( "path" ).datum( parsed ).attr( "fill", "none" )
            .attr( "stroke", color ).attr( "stroke-width", strokeWidth ).attr( "d", lineGen );
    ...
    }, [ data, height, color, yAxisLabel, showGrid, curve, strokeWidth, formatY ] );
