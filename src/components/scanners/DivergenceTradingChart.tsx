...
// ============================================================================
// 🎨 COLORS & THEME - Using Unified ChartConfig
// ============================================================================
...
const THEME = {
  ...
  candleUpBorder: CHART_COLORS.candleUp,
  candleDownBorder: CHART_COLORS.candleDown,
  grid: CHART_COLORS.grid,
  text: CHART_COLORS.textSecondary,
  textBright: CHART_COLORS.text,
  crosshair: CHART_COLORS.crosshair,
  crosshairLabel: 'rgba(255, 255, 255, 0.85)',
  bullishLine: '#22c55e',
  bearishLine: '#ef4444',
  rsiLine: '#2196f3',
...
}: DivergenceTradingChartProps )
{
  ...
  // Use inline style for exact height control
  const heightStyle = typeof height === 'number' ? { height: `${ height }px` } : { height: '400px' };
  ...
  {
    const mainChartHeight = '56%';
    const indicatorChartHeight = '38%';
    const indicatorTop = '58%';
    ...
    return {
      ...
      yAxis: [
        ...
        },
        {
          ...
          splitLine: {
            show: true,
            lineStyle: { color: THEME.grid, type: 'dashed', opacity: 0.4 }
          }
        }
      ],

      // Data Zoom - TradingView style with drag to pan
      dataZoom: [
        {
          type: 'inside',
          xAxisIndex: [ 0, 1 ],
  ...
  const handleMouseDown = useCallback( ( e: React.MouseEvent ) =>
  {
    ...
    // Calculate grid boundaries (match layout split)
    const mainChartBottom = chartHeight * 0.66; // 66% from top
    const indicatorTop = chartHeight * 0.66;
  ...
  return (
    <div
      ref={ containerRef }
      style={ heightStyle }
      className={ `relative select-none ${ widthClass } ${ dragMode === 'pan'
        ? 'cursor-grabbing'
        : ( dragMode === 'yScale' || dragMode === 'indicatorYScale' )
          ? 'cursor-ns-resize'
          : 'cursor-crosshair'
        } ${ className }` }
    ...
    >
      <div className="absolute inset-0 pointer-events-none">
        {/* Horizontal Separator between Main Chart and Indicator */ }
        <div className="absolute left-0 right-0 top-[66%] h-[1px] bg-[#2b403e]/50" />
      </div>
      {/* ECharts */ }
      <ReactECharts
        ref={ ( e ) =>
        ...
        option={ option }
        className="h-full w-full pointer-events-none relative z-[1]"
