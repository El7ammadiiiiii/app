...
interface SearchResultCardProps
{
  ...
  index: number;
  onInsertToChat?: ( text: string, meta?: { sources?: string[] } ) => void;
}
...
export function SearchResultCard ( { result, index, onInsertToChat }: SearchResultCardProps )
{
  ...
  const handleInsert = ( e: React.MouseEvent ) =>
  {
    ...
    if ( onInsertToChat )
    {
      ...
      const source = result.title && result.url ? `${ result.title } — ${ result.url }` : ( result.title || result.url );
      onInsertToChat( text, { sources: [ source ] } );
    }
  };
