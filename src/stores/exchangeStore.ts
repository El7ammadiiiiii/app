...
import type { ExchangeId, Ticker, OrderBook, Trade } from '@/types/exchanges';
...
interface ExchangeStore
{
  ...
  // Active Exchange & Failover state
  activeExchange: ExchangeId;
  exchangePriority: ExchangeId[];
  ...
  // Active Exchange actions
  setActiveExchange: ( exchange: ExchangeId ) => void;
  switchToNextExchange: () => ExchangeId | null;
...
const DEFAULT_PRIORITY: ExchangeId[] = [ ...EXCHANGE_PRIORITY ];

// Lock default exchange to Binance unless user selects otherwise
exchangeOrchestrator.setManualExchange( 'binance' );
...
export const useExchangeStore = create<ExchangeStore>( ( set, get ) => ( {
  ...
  exchangePriority: DEFAULT_PRIORITY,
  ...
  {
    const currentExchange = get().activeExchange;
    const resolvedExchange = exchangeOrchestrator.resolvePreferredExchange( exchange );
    if ( currentExchange !== resolvedExchange )
    {
      // Clear data when switching exchanges to prevent overlap
      set( {
        activeExchange: resolvedExchange,
        tickers: {},
        orderBooks: {},
        trades: {}
      } );
      console.log( `[ExchangeStore] Switched to ${ resolvedExchange }, cleared previous data.` );
    }
  },

  switchToNextExchange: () =>
  {
    const { activeExchange } = get();
    const nextExchange = exchangeOrchestrator.getActiveExchange();
    if ( nextExchange && nextExchange !== activeExchange )
    {
      set( { activeExchange: nextExchange } );
      console.warn( `[ExchangeStore] Failover triggered: Switched from ${ activeExchange } to ${ nextExchange }` );
      return nextExchange;
    }

    console.error( '[ExchangeStore] Failover failed: No healthy exchange available' );
    return null;
  },

  setAutoFailover: ( enabled ) =>
  {
    set( { isAutoFailoverEnabled: enabled } );
    const current = get().activeExchange;
    if ( enabled )
    {
      exchangeOrchestrator.clearManualExchange();
    } else
    {
      exchangeOrchestrator.setManualExchange( current );
    }
  },
...
{
  exchangeOrchestrator.subscribe( ( exchangeId ) =>
  {
    const state = useExchangeStore.getState();
    if ( !state.isAutoFailoverEnabled ) return;
    const current = state.activeExchange;
    if ( current !== exchangeId )
    {
      useExchangeStore.setState( { activeExchange: exchangeId } );
    }
  } );
}
